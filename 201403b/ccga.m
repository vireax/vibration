...
% use csvread() for comma separated values, dlmread() for tab or other symbols
% nds = csvread('nds1.txt'); mbs = csvread('mbs1.txt'); % Case 1
% nds = csvread('nds2.txt'); mbs = csvread('mbs2.txt'); % Case 2
nds = csvread('nds3.txt'); mbs = csvread('mbs3.txt'); % Case 3
...
% custom damage cases
% assignment of each cases needs to be done manually
% Only one line can be enabled corresponding to each case
...
% Retrive exact modal properties
[eig_vec, eig_val, ~, ~ ] = ...
    vibrate(mbs, nds, alpha);

% Embed experiemental noise to modal properties
[Measured_eig_vector, Measured_eig_value] = ...
    measure2(eig_vec, eig_val, noise_vec, noise_val);
...
% NOISE = 0.03; % default
% + or - 3% => total = 6%
err_val = ones(n,1) + (2*rand(n,1)-1)*noise_val;
err_vec = ones(n) + (2*rand(n)-1)*noise_vec;
...
% push spring's stiffness here, replacing elastic support
